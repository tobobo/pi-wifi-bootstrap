...
async def has_wifi_connection():
    ...
    return await is_command_successful("check_wifi", ["bash", f"{this_dir}/scripts/check_wifi_connection.bash"])


async def set_credentials(ssid, psk):
    await stream_with_labeled_output("set_credentials", ["bash", f"{this_dir}/scripts/setup_wifi_client.bash"], {'SSID': ssid, 'PSK': psk})


async def enable_ap():
    logging.info("main: enabling ap")
    await stream_with_labeled_output("enable_ap", ["bash", f"{this_dir}/scripts/setup_wifi_ap.bash"])
